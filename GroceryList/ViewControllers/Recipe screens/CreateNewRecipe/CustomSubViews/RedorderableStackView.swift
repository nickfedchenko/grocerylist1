...
/// docs
@objc
public protocol StackViewReorderDelegate {
    ...
    @objc optional func didEndReordering(_ stackView: UIStackView)
}

public class RedorderableStackView: UIStackView, UIGestureRecognizerDelegate {
    ...
    public weak var reorderDelegate: StackViewReorderDelegate?
    ...
    public var temporaryViewAlpha: CGFloat = 1
    /// The gap created once the long press drag is triggered
    public var dragHintSpacing: CGFloat = 8
    ...
    fileprivate func addLongPressGestureRecognizerForReorderingToView(_ view: UIView) {
        let longPressGR = UILongPressGestureRecognizer(target: self, action: #selector(RedorderableStackView.handleLongPress(_:)))
    ...
    @objc internal func handleLongPress(_ longPressRecognizer: UILongPressGestureRecognizer) {
        ...
        if longPressRecognizer.state == .began {
            ...
            self.actualView = longPressRecognizer.view!
            self.originalPosition = longPressRecognizer.location(in: self)
            self.originalPosition.y -= self.dragHintSpacing
            self.pointForReordering = self.originalPosition
            self.prepareForReordering()
            
        } else if longPressRecognizer.state == .changed {
            
            // Drag the temporaryView
            let newLocation = longPressRecognizer.location(in: self)
            ...
            if midY > self.pointForReordering.y {
                ...
                if let previousView = self.getPreviousViewInStack(usingIndex: index) {
                    ...
                    longPressRecognizer.state == .cancelled ||
                    longPressRecognizer.state == .failed {
            
            self.cleanupUpAfterReordering()
            self.reordering = false
            self.reorderDelegate?.didEndReordering?(self)
        }
    ...
    fileprivate func styleViewsForReordering() {
        ...
        for subview in self.arrangedSubviews {
            if subview != self.actualView {
                ...
                self.temporaryView.layer.borderColor = R.color.action()?.cgColor
                self.temporaryView.layer.borderWidth = 2
                self.temporaryView.layer.shadowColor = UIColor(hex: "7E7975").cgColor
                self.temporaryView.layer.shadowOpacity = 0.35
                self.temporaryView.layer.shadowOffset = .init(width: 0, height: 12)
                self.temporaryView.layer.shadowRadius = 12
            }
        }
    }
    ...
    fileprivate func styleViewsForEndReordering() {
        ...
        square.toValue = self.cornerRadii
        square.duration = 0.1
        ...
        for subview in self.arrangedSubviews {
            UIView.animate(withDuration: 0.3, animations: {
                ...
                subview.layer.cornerRadius = self.cornerRadii
                subview.layer.borderWidth = 0
                subview.layer.shadowColor = UIColor(hex: "858585").cgColor
                subview.layer.shadowOpacity = 0.1
                subview.layer.shadowOffset = .init(width: 0, height: 2)
